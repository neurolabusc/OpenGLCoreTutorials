...
{$ENDIF}
uses
  glcorearb, {$IFDEF GZIP}{$IFDEF FPC}zstream, {$ELSE} zlib,{$ENDIF}{$ENDIF} //Freepascal includes the handy zstream function for decompressing GZip files
...
type
  ...
  tVolRGBA = array of TRGBA;
  TScale = array [1..3] of single;
function Load3DTextures (FileName : AnsiString; var  gradientVolume, intensityVolume  : GLuint; var maxDim: integer; var ScaleDim: TScale; loadGradients: boolean): boolean;// Load 3D image                                                                 }
...
procedure Byte2Byte(var  rawData: tVolB; var lHdr: TNIFTIHdr);
...
begin
  ...
  mx := round((lHdr.cal_max-lHdr.scl_inter)/lHdr.scl_slope);
  mn := round((lHdr.cal_min-lHdr.scl_inter)/lHdr.scl_slope);
  if (mn < 0) or (mx > 255) or (mn = mx) then
   exit;
  ...
  for i := 0 to (vx-1) do begin
      if rawdata[i] <= mn then
         rawdata[i] := 0
      else if rawdata[i] >= mx then
           rawdata[i] := 255
      else
          rawdata[i] := round((rawdata[i]-mn)*Scale);
  end;
end;
...
function Load3DTextures(FileName : AnsiString; var gradientVolume, intensityVolume : GLuint; var maxDim: integer; var ScaleDim: TScale; loadGradients: boolean): boolean;// Load 3D data                                 }
var
 X,Y,Z: integer;
 i: GLint;
 rawData: tVolB;
...
begin
 ...
 if isRGBA = 1 then
  glTexImage3D(GL_PROXY_TEXTURE_3D, 0, GL_RGBA, X, Y, Z, 0, GL_RGBA, GL_UNSIGNED_BYTE, NIL)
     ...
     glTexImage3D(GL_PROXY_TEXTURE_3D, 0, GL_RED, X, Y, Z, 0, GL_RED, GL_UNSIGNED_BYTE, NIL);
 ...
 glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_BORDER);
 glTexParameteri(GL_TEXTURE_3D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_BORDER);
 glTexImage3D(GL_TEXTURE_3D, 0, GL_RED, X, Y, Z, 0, GL_RED, GL_UNSIGNED_BYTE, @rawData[0]);
 CreateGradientVolume (rawData, X, Y, Z, isRGBA, gradientVolume);
 rawData :=nil;
 if (X > Y) then maxDim := X else maxDim := Y;
 if (Z > maxDim) then maxDim := Z;
end;
